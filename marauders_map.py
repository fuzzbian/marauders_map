#!/usr/bin/env python3
"""
This is a modification of the gen_distance_fast-script.
The main difference is that this version uses SVF-tools 
to consider function pointers for the CG generation.
"""
### SET THIS TO THE ABSOLUTE PATH OF SVF-TOOLS ###
SVF = "[PATH]/SVF"                             ###
##################################################
...
SVF_DIR = f"{SVF}/Release-build/bin"
...
# ******************** construct CG with SVF ***********************
def svf_callgraph(args, binary):
    global SVF_DIR
    
    print(f"({STEP}) Constructing CG for {binary} (with SVF)..")
    dot_files = args.temporary_directory / DOT_DIR_NAME

    cmd = [f"{SVF_DIR}/wpa", "-ander", "-dump-callgraph", 
           f"{binary}"]
    ...
    os.remove(f"{dot_files}/callgraph_initial.dot")
    format_cg(args, binary)

# ******************************************************************
# ******** adjust CG-dot-file to expected format *******************
def format_cg(args, binary):
    print(f"Adjusting CG-format of {binary}..")

    dot_files = args.temporary_directory / DOT_DIR_NAME
    prefix = dot_files / f"{binary.name}"
    cg_name = f"{prefix}.callgraph.dot"

    header = f'digraph "Call graph: {args.binaries_directory}/{binary.name}" {{\n'
    label = f'\tlabel="Call graph: {args.binaries_directory}/{binary.name}";\n'

    with open(f"{dot_files}/callgraph_final.dot", 'r') as cg_svf:
        with open(cg_name, 'w') as cg_out:
            # overwrite header and label
            cg_out.write(header)
            cg_out.write(label)

            func_dict = {}  # need this to add "hidden" callsites
            i = 0
            for line in cg_svf:
                new_line = line
                # ignore header and label of svf-generated CG
                if i < 2: 
                    i += 1
                    continue
                
                # re-label nodes and add to func_dict
                is_node = re.search("CallGraphNode ID", line)
                if is_node:
                    # re-label
                    func_name = line.split("\{fun: ")[1].split("\}")[0]
                    new_line = re.sub('label=".*"', f'label="{{{func_name}}}"', line)
                    # add func to func_dict
                    node_id = new_line.split()[0]
                    func_dict[node_id] = func_name

                cg_out.write(new_line)
    os.remove(f"{dot_files}/callgraph_final.dot")
    add_callsites(args, cg_name, func_dict)
# ******************************************************************
# ********* add indirect calls to list of callsites ****************
def add_callsites(args, cg_name, func_dict):
    print(f'Adding new "hidden" callsites from {cg_name}...')
    temp_dir = args.temporary_directory

    
    with open(cg_name, 'r') as cg:
        for line in cg:
            is_hidden_call = re.search("color=red", line)
            if is_hidden_call:
                line = line.strip()
                nodes = line.split(' -> ')
                caller = func_dict[nodes[0].split(':')[0]]
                called = func_dict[nodes[1].split('[')[0]]
                print(f"New hidden call: {caller} -> {called}")

                # get BB from caller-cfg and write to BBCalls.txt
                bb = getBB(args, caller)
                callsite = f"{bb},{called}"
                with open(f"{temp_dir}/BBcalls.txt", 'a') as bbcalls:
                        bbcalls.write(f"{callsite}\n")
    

# ******************************************************************
# **************** extract basic blocks from cfg *******************
def getBB(args, name):
    # note: since we don't know which BB the indirect call 
    #       comes from, we use the first one to represent the func

    dot_files = args.temporary_directory / DOT_DIR_NAME

    with open(f"{dot_files}/cfg.{name}.dot", 'r') as cfg:
        i = 0
        bbs = []
        for line in cfg:
            # skip header
            ...
            is_bb =re.search("label=", line)
            if is_bb:
                bb = line.split('label="{')[1].split(':}"')[0]
                # sanity check
                includes_line = re.search(":[0-9]*", bb)
                if includes_line:
                    bbs.append(bb)

        # pick the start of the func == smallest line number
        smallest = (int(bbs[0].split(':')[1]), bbs[0])
        for bb in bbs:
            if int(bb.split(':')[1]) < smallest[0]:
                smallest = (int(bb.split(':')[1]), bb)
...
def construct_callgraph(args, binaries):
    ...
    for binary in binaries:
        # here we use svf_callgraph instead of opt_callgraph
        # opt_callgraph(args, binary)
        svf_callgraph(args, binary)
